...
func recalculate_vision(override_static_flag = false):
	...
	if not should_recalculate:
		var has_position_changed = (
			_last_position == null or (global_position - _last_position).length() > static_threshold
		)
...
func calculate_vision_shape(override_static_flag = false) -> Array[Vector2]:
	...
	for i in range(ray_count + 1):
		...
		var p = _ray_to(
			Vector2(0, max_distance).rotated(_angular_delta * i + global_rotation - _angle_half)
		)
...
func _ray_to(direction: Vector2) -> Vector2:
	...
	var query = PhysicsRayQueryParameters2D.create(
		global_position, destination, collision_layer_mask
	)
